...
class AddUserUseCaseTest {
    ...
    @Test
    fun `get true - user added`() {
        val result = Resource.Success(true)

        coEvery { userRepository.addUser(any()) } returns flowOf(result)

        val response = runBlocking { addUserUseCase(user).first() }

        coVerify(exactly = 1) { userRepository.addUser(user) }
    ...
    @Test
    fun `return error`() {
        coEvery { userRepository.addUser(any()) } returns flowOf(Resource.Error("Error message"))
    ...
    fun `addUser is loading`() {
        coEvery { userRepository.addUser(any()) } returns flowOf(Resource.Loading(true))
